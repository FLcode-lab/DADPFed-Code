...
parser.add_argument('--dataset', choices=['CIFAR10', 'CIFAR100', 'face_dataset', 'hmdb51', 'UCF101-MA'], type=str, default='CIFAR10')             # select dataset
...
parser.add_argument('--batchsize', default=16, type=int)                                                   # select the batchsize
...
parser.add_argument('--momentum', default=0.9, type=float)                                                 # local SGD momentum

# DADPFed
parser.add_argument('--dadpfed-cycle', default=40, type=int)                                               # communication cycle R
parser.add_argument('--dadpfed-retention', default=0.75, type=float)                                       # retention ratio Th
parser.add_argument('--dadpfed-mask-quantile', default=0.75, type=float)                                   # quantile for drift-aware mask

parser.add_argument('--method', choices=['FedAvg', 'FedCM', 'FedDyn', 'SCAFFOLD', 'FedAdam', 'FedProx', 'FedSAM', 'MoFedSAM', \
                                         'FedGamma', 'FedSpeed', 'FedSMOO', 'DADPFed', 'DADPFedSAM'], type=str, default='FedAvg')
...
if __name__=='__main__':
    ...
    if args.dataset == 'CIFAR10':
        ...
        classes = 100
    #face_classification
    elif args.dataset == 'face_dataset':
        classes = 8
    elif args.dataset == 'hmdb51':
        classes = 50
    elif args.dataset == 'UCF101-MA':
        classes = data_obj.n_cls
    else:
    ...
    if args.method == 'FedAvg':
        ...
        server_func = FedSMOO
    elif args.method == 'DADPFed':
        server_func = DADPFed
    elif args.method == 'DADPFedSAM':
        server_func = DADPFedSAM
    else:
